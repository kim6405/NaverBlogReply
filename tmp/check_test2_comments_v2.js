...
(async () => {
    ...
    const data = await p.evaluate(() => {
        return Array.from(document.querySelectorAll('.u_cbox_comment')).map(el => {
            ...
            const isOwner = el.innerText.includes('블로그주인');
            const content = el.querySelector('.u_cbox_contents')?.innerText || "";
            return { isReply, isOwner, content: content.substring(0, 30) };
        });
    });
