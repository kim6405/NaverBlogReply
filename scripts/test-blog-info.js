...
async function test() {
    ...
    const neighborBlogId = 'dung8825'; // let's use an arbitrary blogId
    
    await page.goto(`https://m.blog.naver.com/${neighborBlogId}?listStyle=card`, { waitUntil: "networkidle" });
    ...
    const info = await page.evaluate(() => {
        ...
        const links = Array.from(document.querySelectorAll('a')).map(l => {
            ...
            // Parse as string instead of int to avoid precision loss just in case? Or use parseInt.
